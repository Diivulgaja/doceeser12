...
// --- COMPONENTES (Definidos fora para evitar recriação) ---
...
export default function App() {
  ...
  const handleAuth = async (type, data) => {
    ...
    try {
      if (type === 'login') {
        ...
        const newUserProfile = { name: data.name, phone: data.phone, password: data.password, address: {}, createdAt: new Date().toISOString() };
  ...
  const loginUser = (userData) => {
    ...
    setCustomer(prev => ({ ...prev, nome: userData.name, telefone: userData.phone, rua: userData.address?.rua || '', numero: userData.address?.numero || '', bairro: userData.address?.bairro || '' }));
  ...
  const handleInitiatePayment = async () => {
    if (!customer.nome || !customer.telefone || !customer.rua) return alert("Por favor, preencha seus dados de entrega.");
    if (user && supabase) {
          ...
          <div className="pb-32">
            ...
            <div className="max-w-4xl mx-auto px-4">
              <div className="flex items-center justify-between mb-6">
                 ...
                 <span className="text-xs font-medium text-gray-400">{initialProducts.filter(p => categoryFilter === 'all' || p.category === categoryFilter).length} itens</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {initialProducts.filter(p => categoryFilter === 'all' || p.category === categoryFilter).map(p => <ProductCard key={p.id} product={p} onAdd={addToCart} onOpenAcai={setAcaiModalProduct} />)}
              </div>
            </div>
          </div>
        )}
